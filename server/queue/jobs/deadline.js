  ...
  dueCards.forEach((card) => {
    template += `
      <div style="background-color: #eee; padding: 20px;">
        <p>${card.name}</p>
        <p>Due ${new Intl.DateTimeFormat("en-US").format(
...
module.exports = async (job, done) => {
  ...
  connectDB();
  const boards = await Board.find().populate("user", User);
  ...
  const mappedBoard = boards.map((board) => {
    return {
      ...
      cards: board.columns
        .map((col) =>
          col.cards.filter(
            (card) => new Date().getDay() === new Date(card.dueDate).getDay()
          )
        )
        .flat(),
    };
  ...
  mongoose.disconnect(() =>
    console.log("Disconnected mongo connection in spawned process")
  );
